"""Python library for controlling the TLB-6700 Tunable Laser Controller via Newport USB DLL."""
...
class NewportUSB:
    ...
    def __init__(self, dll_path=None):
        if not self._initialized:
            try:
                ...
                raise RuntimeError("Could not load UsbDll.dll. Ensure Newport USB Driver is installed.")
    ...
    def _setup_functions(self):
        ...
        self.dll.newp_usb_send_ascii.argtypes = [ctypes.c_long, ctypes.c_char_p, ctypes.c_ulong]
        self.dll.newp_usb_send_ascii.restype = ctypes.c_long
        
        self.dll.newp_usb_get_ascii.argtypes = [ctypes.c_long, ctypes.c_char_p, ctypes.c_ulong, ctypes.POINTER(ctypes.c_ulong)]
    ...
    def init_system(self):
        ...
        if result != 0:
            raise RuntimeError(f"Failed to initialize USB system: error code {result}")
    ...
    def list_devices(self):
        ...
        if result != 0:
            raise RuntimeError(f"Failed to get device info: error code {result}")
